...
pub fn process_questdb_batch(args: &Args) -> Result<()> {
    ...
    for symbol in &symbols {
        ...
        for (tf, candles) in chain {
            ...
            let out_file = out_dir.join(format!("questdb_{}.csv", format!("{:?}", tf)));
            let io_start = std::time::Instant::now();
